...
const reverHandler = ()  => {
 ...
 useEffect(() => {
  ...
  return  (isGoingTobeFired &&
    <div role='timerWrapepr' className='h-12 w-full flex flex-row justify-between items-center bg-gray bg-opacity-30'>  
        <span className='font-vazir-adad text-sm text-text mx-5'>
       {`  00:00:${collapsingTime}`}    ثانیه برای بازگرداندن تسک فرصت دارید 
        </span>
        <label role='timerExecuter' onClick={() => reverHandler()} className='mx-5 cursor-pointer'>
        <GrRevert/>
        </label>
    </div>
  )
